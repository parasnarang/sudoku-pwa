class AppRouter {
    ...
    updateProfileStats() {
        ...
        let level = 'Beginner';
        if (gamesCompleted >= 100) level = 'Expert';
        else if (gamesCompleted >= 50) level = 'Advanced';
        else if (gamesCompleted >= 20) level = 'Intermediate';
        else if (gamesCompleted >= 5) level = 'Novice';
        ...
        document.getElementById('backup-data-btn').addEventListener('click', async () => {
            ...
            try {
                ...
                if (backup.success) {
                    ...
                    link.download = `sudoku-backup-${new Date().toISOString().split('T')[0]}.json`;
                    link.click();
